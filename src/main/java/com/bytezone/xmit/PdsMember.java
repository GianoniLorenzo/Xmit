...
public class PdsMember extends DataFile implements Iterable<DataBlock> {
  ...
  public void listSizeCounts(List<String> lines) {
    ...
    for (SizeCount sizeCount : sizeCounts.values()) {
      ...
      lines.add(
          String.format(
              " %,5d   %,7d    %,5d  %,9d", ++seq, sizeCount.blockSize, sizeCount.count, total));
  ...
  public byte[] getDataBuffer() {
    ...
    for (DataBlock dataBlock : dataBlocks) ptr = dataBlock.packBuffer(buffer, ptr);
  ...
  public byte[] getDataBuffer(int limit) {
    ...
    if (getDataLength() <= limit) return getDataBuffer();
    ...
    for (DataBlock dataBlock : tmpBlocks) ptr = dataBlock.packBuffer(buffer, ptr);
  ...
  boolean isRdw() {
    ...
    for (DataBlock dataBlock : dataBlocks)
      if (dataBlock.getTwoBytes() != dataBlock.getSize()) return false;
  ...
  void rdwLines() // see SOURCE.XMI
      {
    for (DataBlock dataBlock : dataBlocks) {
      if (dataBlock.getSize() == 0) continue;
      ...
      while (ptr < buffer.length) {
        ...
        if (isBinary) {
          ...
          lines.add("");
        } else lines.add(Utility.translateUnicode(buffer, ptr + 4, len - 4).stripTrailing());
  ...
  void undefined() // recfm = U
      {
    if (catalogEntry == null || catalogEntry.isLoadModule() || getCommonBlockLength() <= 1) {
      if (getEightBytes()[0] == 0x20) loadModule();
      else hexDump();
    } else for (DataBlock block : dataBlocks) lines.add(Utility.getString(block.getBuffer()));
  }
  ...
  public void getText(List<String> lines) {
    ...
    lines.add("");
    lines.add("    #    Offset    Header                    Data         Data      Ptrs");
    lines.add("  ----  --------  -------------------------- --------  ---------  ------");
    for (DataBlock dataBlock : dataBlocks) {
    ...
    lines.add(String.format("%44.44s %02X %02X %02X %,10d  %,6d", "", b1, b2, b3, total, pointers));
