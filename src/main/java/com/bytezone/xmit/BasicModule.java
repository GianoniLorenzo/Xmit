...
public class BasicModule extends CatalogEntry {
  ...
  BasicModule(byte[] buffer) {
    ...
    //    if (buffer.length == 12)          // hw == 0
    {
      ...
      if (buffer.length >= 20) {
        ...
        if (opt.isPresent()) dateCreated = opt.get();
      }

      if (buffer.length >= 24) {
        Optional<LocalDate> opt = Utility.getLocalDate(buffer, 20);
        if (opt.isPresent()) dateModified = opt.get();
      }
      ...
      if (buffer.length >= 40) userName = Utility.getString(buffer, 32, 8).trim();
    }
  }
  ...
  public static String getDebugHeader() {
    ...
    return CatalogEntry.getDebugHeader()
        + "-- id -- vv mm 00 ss -created--  -modified-"
  ...
  public String toString() {
    ...
    String date1Text =
        dateCreated == null ? "" : String.format("%td %<tb %<tY", dateCreated).replace(".", "");
    return String.format(
        "%s  %-8s  %-8s  %,6d  %s  %s",
        isAlias() ? "A" : ".", getMemberName(), userName, size, date1Text, time);
  }
}
