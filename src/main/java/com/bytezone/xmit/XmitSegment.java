...
class XmitSegment extends Segment {
  ...
  List<DataBlock> createDataBlocks() // used only for data blocks
    ...
    if (debug) {
      ...
      for (BlockPointer blockPointer : rawBlockPointers) System.out.println(blockPointer);
    }
    ...
    if (debug) {
      ...
      for (DataBlock block : dataBlocks) System.out.println(block);
    }
  ...
  public byte[] getRawBuffer() // contains no headers
    ...
    for (BlockPointer blockPointer : rawBlockPointers) {
      System.arraycopy(
          blockPointer.buffer, blockPointer.offset, fullBlock, ptr, blockPointer.length);
  ...
  int packBuffer(byte[] dataBuffer, int ptr) {
    ...
    for (BlockPointer blockPointer : rawBlockPointers) {
      System.arraycopy(
          blockPointer.buffer, blockPointer.offset, dataBuffer, ptr, blockPointer.length);
      ptr += blockPointer.length;
    }
