...
class DataBlock {
  ...
  byte[] getEightBytes() {
    ...
    if (header.getSize() == 0) return eightBytes;
  ...
  byte[] getSixteenBytes() {
    ...
    if (header.getSize() == 0) return sixteenBytes;

    BlockPointer blockPointer = blockPointers.get(0);
    System.arraycopy(
        blockPointer.buffer, blockPointer.offset, sixteenBytes, 0, sixteenBytes.length);
  ...
  byte[] getBuffer() {
    ...
    for (BlockPointer blockPointer : blockPointers) {
      System.arraycopy(blockPointer.buffer, blockPointer.offset, buffer, ptr, blockPointer.length);
  ...
  int packBuffer(byte[] buffer, int offset) {
    ...
    for (BlockPointer blockPointer : blockPointers) {
      System.arraycopy(
          blockPointer.buffer, blockPointer.offset, buffer, offset, blockPointer.length);
  ...
  String getDetails() {
    ...
    if (blockPointers.size() > 0) {
      for (BlockPointer blockPointer : blockPointers) text.append(blockPointer + "\n");
  ...
  public String toString() {
    ...
    return String.format("%08X  %s   %,7d  %,6d", offset, header, getSize(), blockPointers.size());
  }
}
