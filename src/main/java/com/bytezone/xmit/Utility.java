...
public class Utility {
  ...
  private static final Map<String, CodePage> codePageMap = new HashMap<>();
  ...
  public static void setCodePage(String codePageName) {
    ...
    if (codePageMap.containsKey(codePageName)) codePage = codePageMap.get(codePageName);
    else {
  ...
  public static FileType getFileType(byte[] buffer) {
    ...
    for (byte[] signature : signatures) {
      if (matches(signature, buffer, 0)) return FileType.values()[count];
      ++count;
    }
  ...
  public static String getString(byte[] buffer, int ptr, int length) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      else text.append(c < 0x40 || c == 0xFF ? "." : (char) codePage.ebc2asc[c]);
    }
  ...
  static String translateUnicode(String s) {
    ...
    for (int i = 1; i < chunks.length; i++) {
      text.append(Character.toString(Integer.parseInt(s.substring(ptr + 2, ptr + 5))));
  ...
  private static boolean isUnicode(byte[] buffer, int i) {
    ...
    if (i + 5 >= buffer.length) return false;
    if (buffer[i++] != (byte) 0xE0) return false;
    if (buffer[i++] != (byte) 0xA4) return false;
    for (int j = 0; j < 3; j++) if (!isDigit(buffer[i++] & 0xFF)) return false;
    return buffer[i] == (byte) 0x40;
  }
  ...
  public static boolean matches(byte[] key, byte[] buffer, int ptr) {
    ...
    if (ptr + key.length > buffer.length) return false;

    for (int i = 0; i < key.length; i++) if (key[i] != buffer[ptr + i]) return false;
  ...
  public static boolean matches(byte[] key, int ptr1, byte[] buffer, int ptr2, int length) {
    ...
    if (ptr1 + length > key.length || ptr2 + length > buffer.length) return false;

    for (int i = 0; i < length; i++) if (key[ptr1 + i] != buffer[ptr2 + i]) return false;
  ...
  public static String stripLineNumber(String line)
        // ----------------------------------------------------------------------------------- //
      {
    if (line.length() < 72 || line.length() > 80) return line;
    String numbers = line.substring(72);
    for (char c : numbers.toCharArray()) if ((c < '0' || c > '9') && c != ' ') return line;
  ...
  public static long getFourBytes(byte[] buffer, int ptr) {
    ...
    return ((long) (buffer[ptr] & 0xFF) << 24)
        | ((buffer[++ptr] & 0xFF) << 16)
        | ((buffer[++ptr] & 0xFF) << 8)
        | (buffer[++ptr] & 0xFF);
  }
  ...
  public static long getValue(byte[] buffer, int offset, int length) {
    ...
    long value = 0;
    while (length-- > 0) value = value << 8 | buffer[offset++] & 0xFF;
    return value;
  }
  ...
  public static Optional<LocalDate> getLocalDate(byte[] buffer, int offset) {
    ...
    String date1 =
        String.format(
            "%02X%02X%02X%02X",
            buffer[offset], buffer[offset + 1], buffer[offset + 2], (buffer[offset + 3] & 0xF0));
    try {
  ...
  public static String getHexValuesWithText(byte[] buffer, int offset, int length) {
    ...
    for (int i = offset, max = offset + length; i < max; i++) {
      ...
      else text.append(String.format("%02X ", c));
    }
    return text.toString();
  }
  ...
  public static String getHexDump(byte[] b, int offset, int length, int displayOffset) {
    ...
    for (int ptr = offset, max = offset + length; ptr < max; ptr += lineSize) {
      ...
      text.append(String.format("%06X  %-48s %s%n", displayOffset + ptr, hexLine, textLine));
    }
  ...
  public static List<String> getHexDumpLines(byte[] b, int offset, int length, int displayOffset) {
    ...
    for (int ptr = offset, max = offset + length; ptr < max; ptr += lineSize) {
      ...
      lines.add(String.format("%06X  %-48s %s", displayOffset + ptr, hexLine, textLine));
    }
  ...
  static int find(byte[] haystack, byte[] needle)
    ...
    for (int i = 0; i <= haystack.length - needle.length; ++i) {
      for (int j = 0; j < needle.length; ++j) if (haystack[i + j] != needle[j]) continue loop;
  ...
  private static String getFirstNonComment(List<String> lines)
        // ----------------------------------------------------------------------------------- //
      {
    for (String line : lines) if (!line.startsWith("//*")) return line;
    return "";
  }
  ...
  public enum FileType {
    ...
    AFP,
    BIN,
    XMI // must match signatures array
  }
}
