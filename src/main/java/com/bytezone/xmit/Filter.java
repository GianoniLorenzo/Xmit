...
public class Filter {
  ...
  public List<CatalogEntry> getCatalogEntries(FilterMode filterMode) {
    ...
    // nothing to do
    if (filterMode == FilterMode.OFF || key.isEmpty()) return pdsDataset.getCatalogEntries();
    ...
    if (filtered == null) {
      ...
      for (CatalogEntry catalogEntry : pdsDataset.getCatalogEntries())
        if (catalogEntry.contains(key)) filtered.add(catalogEntry);
        else reversed.add(catalogEntry);
    }
  ...
  public String toString() {
    ...
    if (filtered == null)
      return String.format("Filter: %-10s  %,d", key, pdsDataset.getCatalogEntries().size());

    return String.format(
        "Filter: %-10s %,d + %,d = %,d",
        key, filtered.size(), reversed.size(), pdsDataset.getCatalogEntries().size());
  }
