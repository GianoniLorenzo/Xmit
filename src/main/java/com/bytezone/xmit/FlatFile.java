...
public class FlatFile extends DataFile implements Iterable<Segment> {
  ...
  public byte[] getDataBuffer() {
    ...
    if (true) return getDataBufferRDW();

    byte[] buffer = new byte[getDataLength()];
    int ptr = 0;

    for (Segment segment : segments) ptr = segment.packBuffer(buffer, ptr);
  ...
  public byte[] getDataBuffer(int limit) {
    ...
    for (Segment segment : tmpSegments) ptr = segment.packBuffer(buffer, ptr);
