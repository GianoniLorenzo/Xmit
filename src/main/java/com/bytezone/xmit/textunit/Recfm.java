...
public class Recfm extends TextUnitNumber {
  ...
  public Recfm(byte[] buffer, int ptr) {
    ...
    if (number == 0xC000) type = "U";
    else {
      if ((number & 0x8000) != 0) type = "F";
      else if ((number & 0x4000) != 0) type = "V";

      if ((number & 0x1000) != 0) type += "B";

      if ((number & 0x0400) != 0 || (number & 0x0200) != 0) type += "A";

      if ((number & 0x0800) != 0) {
        if (type.startsWith("V")) type += "S"; // spanned
        else if (type.startsWith("F")) type += " last block may be short";
      }

      if ((number & 0x0002) != 0) type += " (no 4-byte header)";

      if ((number & 0x0001) != 0) type = "Shortened VBS format used for transmission records";
    }
    ...
    if (false) {
      ...
      System.out.printf("%04X  %s  %s  %s%n", number, types1[t1], types2[t2], types3[t3]);
    }
  }
  ...
  public String toString() {
    ...
    return type.isEmpty()
        ? super.toString()
        : String.format("%04X  %-8s  %04X  %s", keys[keyId], mnemonics[keyId], number, type);
  }
}
