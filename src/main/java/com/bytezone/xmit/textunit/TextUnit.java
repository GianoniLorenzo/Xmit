...
public class TextUnit {
  ...
  static int[] keys = {
    0x0000, INMBLKSZ, INMCREAT, INMDDNAM, INMDIR, INMDSNAM, INMDSORG, INMEATTR, INMERRCD, INMEXPDT,
    INMFACK, INMFFM, INMFNODE, INMFTIME, INMFUID, INMFVERS, INMLCHG, INMLRECL, INMLREF, INMLSIZE,
    INMMEMBR, INMNUMF, INMRECCT, INMRECFM, INMSECND, INMSIZE, INMTERM, INMTNODE, INMTTIME, INMTUID,
    INMTYPE, INMUSERP, INMUTILN
  };
  ...
  static String[] descriptions = {
    ...
    "RECEIVE command error code",
    "Expiration date",
    "Originator requested notification",
    "Filemode number",
    "Origin node name or node number",
    "Origin timestamp",
    "Origin user ID",
    ...
    "Transmitted record count",
    "Record format",
    "Secondary space quantity",
    "File size in bytes",
    "Data transmitted as a message",
    "Target node name or node number",
    "Destination timestamp",
    "Target user ID",
    "Data set type",
    "User parameter string",
    "Name of utility program"
  };
  ...
  public TextUnit(byte[] buffer, int ptr) {
    ...
    if (keyId == 0) System.out.printf("Unknown key:: %04X%n", key);
  }
  ...
  private boolean findKey(int keyId) {
    ...
    for (int key : keys) if (key == keyId) return true;
    return false;
  }
  ...
  public String toString() {
    ...
    text.append(String.format("%04X  %-8s  %04X", keys[keyId], mnemonics[keyId], dataList.size()));

    for (Data data : dataList) text.append(String.format("  %s", data) + ",");

    if (dataList.size() > 0) text.deleteCharAt(text.length() - 1);
