...
class OutputTab extends XmitTextTab
  ...
  private List<String> getLines(int maxLines) {
    ...
    for (String line : lines) {
      ...
      if (lineDisplayStatus.stripLines) line = Utility.stripLineNumber(line);

      if (lineDisplayStatus.truncateLines && line.length() > 0) newLines.add(line.substring(1));
      else newLines.add(line);

      if (isJCL) includeDatasetName = checkInclude(includeDatasetName, line, newLines);
    }
  ...
  private String checkInclude(String includeDatasetName, String line, List<String> newLines) {
    ...
    if (!includeDatasetName.isEmpty()) {
      ...
      if (m.find()) append(newLines, includeDatasetName, m.group(2), "//*");
    }

    Matcher m = includePattern.matcher(line);
    if (m.find()) includeDatasetName = m.group(1);

    return includeDatasetName;
  }

  private void append(
      List<String> newLines, String datasetName, String memberName, String commentIndicator) {
    ...
    if (optMember.isEmpty())
      ...
      for (String line : optMember.get().getLines())
        if (!line.startsWith(commentIndicator)) newLines.add(line);
  }

  Optional<PdsMember> findMember(String datasetName, String memberName) {

    if (datasets.containsKey(datasetName)) return datasets.get(datasetName).findMember(memberName);
  ...
  public void write(File file) {
    ...
    try (BufferedWriter output = new BufferedWriter(new FileWriter(file))) {
      for (String line : getLines(0)) output.write(line + "\n");
  ...
  public void treeNodeSelected(TreeNodeData nodeData) {
    ...
    if (nodeData.isPartitionedDataset()) {
      ...
      if (!datasets.containsKey(datasetName)) datasets.put(datasetName, (PdsDataset) dataset);
      dataFile = null;
    } else if (nodeData.isPhysicalSequentialDataset()) dataFile = nodeData.getDataFile();
    else dataFile = null;
