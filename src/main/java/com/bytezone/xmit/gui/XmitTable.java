...
class XmitTable extends TableView<CatalogEntryItem> //
  ...
  XmitTable() {
    ...
    for (DataColumn<?> dataColumn : dataColumns) getColumns().add(dataColumn.createColumn());
  ...
  private void selected(
      ObservableValue<? extends CatalogEntryItem> obs,
      CatalogEntryItem oldSel,
  ...
  private CatalogEntryItem find(String name) {
    ...
    for (CatalogEntryItem catalogEntryItem : items)
      if (name.equals(catalogEntryItem.getMemberName())) return catalogEntryItem;
    return null;
  }

  void addFilterListener(FilterActionListener listener) {

    if (!filterListeners.contains(listener)) filterListeners.add(listener);
  }
  ...
  public void setFilter(FilterStatus filterStatus) {
    ...
    if (filterStatus.matches(this.filterStatus)) return;
  ...
  private void buildList(String selectedName) {
    ...
    if (filterStatus.filterValue.isEmpty()) setPlaceholder(new Label("No members to display"));
    else
      setPlaceholder(new Label(String.format("No members contain '%s'", filterStatus.filterValue)));

    // create filter
    Filter filter = ((PdsDataset) nodeData.getDataset()).getFilter(filterStatus.filterValue);
    FilterMode filterMode =
        filterStatus.filterValue.isEmpty() || !filterStatus.filterActive
            ? FilterMode.OFF
  ...
  private CatalogEntryItem findItem(String name) {
    ...
    if (!name.isEmpty())
      for (CatalogEntryItem catalogEntryItem : items)
        if (catalogEntryItem.getMemberName().equals(name)) return catalogEntryItem;
    return null;
  }
  ...
  private void selectCatalogEntryItem(CatalogEntryItem catalogEntryItem) {
    ...
    if (catalogEntryItem == null) getSelectionModel().select(0); // select by index
    else getSelectionModel().select(catalogEntryItem); // select by item
