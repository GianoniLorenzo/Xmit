...
class FileMenu extends Menu implements TableItemSelectionListener, TreeNodeListener, SaveState {
  ...
  public FileMenu(String name) {
    ...
    getItems()
        .addAll(
            rootMenuItem, extractMenuItem, saveMenuItem, new SeparatorMenuItem(), aboutMenuItem);

    rootMenuItem.setAccelerator(new KeyCodeCombination(KeyCode.R, KeyCombination.SHORTCUT_DOWN));
    extractMenuItem.setAccelerator(new KeyCodeCombination(KeyCode.E, KeyCombination.SHORTCUT_DOWN));
  ...
  private void saveFile() {
    ...
    if (!saveFolder.exists()) saveFolder = new File(System.getProperty("user.home"));
  ...
  public void restore(Preferences prefs) {
    ...
    if (!new File(saveFolderName).exists()) saveFolderName = System.getProperty("user.home");

    extractFolderName = prefs.get(PREFS_EXTRACT_FOLDER, System.getProperty("user.home"));
    if (!new File(extractFolderName).exists()) extractFolderName = System.getProperty("user.home");
  }
