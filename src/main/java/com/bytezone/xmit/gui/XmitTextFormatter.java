...
class XmitTextFormatter implements TextFormatter {
  ...
  public List<Text> format(List<String> lines) {
    ...
    if (filterStatus.filterValue.isEmpty() || !filterStatus.filterActive) plainFormat(lines);
    else filterFormat(lines);
  ...
  void plainFormat(List<String> lines) {
    ...
    for (String line : lines) {
      if (showLineNumbers) addText(String.format("%06d ", lineNo++), numberColor);
  ...
  void filterFormat(List<String> lines) {
    ...
    for (String line : lines) {
      ...
      if (highlight(line, filterStatus.filterValue, Color.RED)) continue;

      if (filterStatus.filterExclusion) {
        if (showLineNumbers) textList.remove(textList.size() - 1);
      } else addTextNewLine(line, baseColor);
    }
  ...
  boolean highlight(String line, String text, Color color) {
    ...
    } else addTextNewLine(line.substring(pos), color);
  ...
  boolean highlightAfter(String line, String text, Color color) {
    ...
    if (pos2 < 0) pos2 = line.indexOf(' ', pos);

    if (pos2 < 0) addTextNewLine(line.substring(pos), color);
    else {
      addText(line.substring(pos, pos2), color);
      addTextNewLine(line.substring(pos2), baseColor);
    }
  ...
  void removeLastNewLine() {
    ...
    if (line.length() == 1) textList.remove(textList.size() - 1);
    else text.setText(line.substring(0, line.length() - 1));
  }
}
