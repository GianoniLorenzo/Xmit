...
class TreeNodeData implements Iterable<Dataset> {
  ...
  static final List<String> compressionSuffixes = List.of("zip");
  ...
  boolean isXmit() {
    ...
    return "xmi".equals(suffix) || "xmit".equals(suffix) || (reader != null && reader.isXmit());
  }
  ...
  int size() {
    ...
    if (!isDatasetContainer()) return 0;
  ...
  public String toString() {
    ...
    if (isDatasetContainer()) text.append(String.format(" datasets ...... %s%n", size()));
    ...
    if (isDataset()) {
      ...
      text.append(String.format(" isSequential .. %s%n", isPhysicalSequentialDataset()));
      if (isPartitionedDataset()) {
        text.append(
            String.format(
                " members ....... %s%n", ((PdsDataset) dataset).getCatalogEntries().size()));
        text.append(
            String.format(
                " xmit members .. %s%n", ((PdsDataset) dataset).getPdsXmitMembers().size()));
      }
    }

    text.append(String.format("isMember ....... %s%n", isMember()));

    if (isMember()) text.append(String.format(" member ........ %s%n", member.getName()));
