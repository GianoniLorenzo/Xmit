...
class ViewMenu extends Menu implements SaveState {
  ...
  private CheckMenuItem addCheckMenuItem(
      String name, KeyCode keyCode, EventHandler<ActionEvent> action) {

    return addCheckMenuItem(name, keyCode, NO_SHIFT, action);
  }

  private CheckMenuItem addCheckMenuItem(
      String name, KeyCode keyCode, boolean shift, EventHandler<ActionEvent> action) {
      ...
      if (shift)
        menuItem.setAccelerator(
            new KeyCodeCombination(
                keyCode, KeyCombination.SHORTCUT_DOWN, KeyCombination.SHIFT_DOWN));
      else menuItem.setAccelerator(new KeyCodeCombination(keyCode, KeyCombination.SHORTCUT_DOWN));
  ...
  private void notifyLinesListeners() {
    ...
    for (ShowLinesListener listener : showLinesListeners) listener.showLinesSelected(copy);
  }
  ...
  private void notifyCodePageListeners() {
    ...
    if (toggle == null) {
      ...
      if (!codePageName.startsWith("USER") && !Charset.isSupported(codePageName))
        AppBase.showAlert(
            AlertType.ERROR, "Encoding Exception", "Charset " + codePageName + " is not supported");
      else {
  ...
  public void save(Preferences prefs) {
    ...
    Toggle toggle = toggleGroup.getSelectedToggle();
    if (toggle != null) prefs.put(PREFS_CODE_PAGE, toggle.getUserData().toString());
  }

  public void addShowLinesListener(ShowLinesListener listener) {

    if (!showLinesListeners.contains(listener)) showLinesListeners.add(listener);
  }

  public void addCodePageListener(CodePageSelectedListener listener) {

    if (!codePageListeners.contains(listener)) codePageListeners.add(listener);
  }
}
