...
class TableHeaderBar extends HeaderBar
  ...
  public void treeNodeSelected(TreeNodeData nodeData) {
    ...
    if (nodeData.isDataset()) {
      ...
      leftLabel.setTextFill(nodeData.getReader().isIncomplete() ? Color.RED : Color.BLACK);
      setMembersLabel();
    } else {
      leftLabel.setText("");
      rightLabel.setText("");
  ...
  public void filtering(int found, int max, boolean done) {
    ...
    if (!done || filterStatus.filterValue.isEmpty()) return;
  ...
  private void setMembersLabel() {
    ...
    if (nodeData != null && nodeData.isPartitionedDataset()) {
      ...
      if (filterStatus.filterValue.isEmpty() || !filterStatus.filterActive)
        rightLabel.setText(String.format("%d member%s", members, members == 1 ? "" : "s"));
      else rightLabel.setText(String.format("%d / %d member%s", found, max, max == 1 ? "" : "s"));
    } else rightLabel.setText("");
  }
}
