...
abstract class DataColumn<T> implements Comparable<DataColumn<?>> {
  ...
  public DataColumn(
      String heading, String name, int width, String alignment, DisplayType displayType) {
    ...
    savedWidth = prefs.getDouble(PREFS_WIDTH + columnHeading, widthInCharacters * PIXELS_PER_CHAR);
  ...
  public String toString() {
    ...
    return String.format(
        "%2d  %-15s %-15s %4d  %s",
        sequence, columnHeading, propertyName, widthInCharacters, displayType);
  }
...
class StringColumn extends DataColumn<String> {
  ...
  Callback<TableColumn<CatalogEntryItem, String>, TableCell<CatalogEntryItem, String>>
      createCallback() {
    return new Callback<
        TableColumn<CatalogEntryItem, String>, //
        TableCell<CatalogEntryItem, String>>() {
      @Override
      public TableCell<CatalogEntryItem, String> call(TableColumn<CatalogEntryItem, String> param) {
        TableCell<CatalogEntryItem, String> cell =
            new TableCell<>() {
              @Override
...
// ---------------------------------------------------------------------------------//
{
  ...
  Callback<TableColumn<CatalogEntryItem, Number>, TableCell<CatalogEntryItem, Number>>
      createCallback() {
    return new Callback<
        TableColumn<CatalogEntryItem, Number>, //
        TableCell<CatalogEntryItem, Number>>() {
      @Override
      public TableCell<CatalogEntryItem, Number> call(TableColumn<CatalogEntryItem, Number> param) {
        TableCell<CatalogEntryItem, Number> cell =
            new TableCell<>() {
              @Override
              public void updateItem(final Number item, boolean empty) {
                ...
                if (item == null || empty) setText(null);
                  ...
                  if (item.intValue() == 0) setText("");
                  else setText(String.format(mask, item));
                  setFont(font);
                }
              }
            };
...
class LocalDateColumn extends DataColumn<LocalDate> {
  ...
  Callback<TableColumn<CatalogEntryItem, LocalDate>, TableCell<CatalogEntryItem, LocalDate>>
      createCallback() {
    return new Callback<
        TableColumn<CatalogEntryItem, LocalDate>, //
        TableCell<CatalogEntryItem, LocalDate>>() {
      @Override
      public TableCell<CatalogEntryItem, LocalDate> call(
          TableColumn<CatalogEntryItem, LocalDate> param) {
...
class FileTypeColumn extends DataColumn<FileType> {
  ...
  Callback<TableColumn<CatalogEntryItem, FileType>, TableCell<CatalogEntryItem, FileType>>
      createCallback() {
    return new Callback<
        TableColumn<CatalogEntryItem, FileType>, //
        TableCell<CatalogEntryItem, FileType>>() {
      @Override
      public TableCell<CatalogEntryItem, FileType> call(
          TableColumn<CatalogEntryItem, FileType> param) {
