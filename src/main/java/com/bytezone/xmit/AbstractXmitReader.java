...
public class AbstractXmitReader extends Reader implements XmitReader {
  ...
  private void read(byte[] buffer) {
    ...
    while (ptr < buffer.length) {
      ...
      if (isRecordNumber) System.out.println("******** Found a record number");

      if (isFirstSegment) currentSegment = new XmitSegment();
      ...
      if (isLastSegment) {
        if (isControlRecord) {
          ControlRecord controlRecord = new ControlRecord(currentSegment.getRawBuffer());
          ...
          switch (controlRecord.getControlRecordType()) {
            ...
            case INMR03:
              ...
              if (optOrg.isEmpty()) throw new IllegalArgumentException("DSORG not found");
        ...
        else currentDataset.addSegment(currentSegment);
      } // isLastSegment
    } // while ptr < buffer.length

    // allocate the data records
    for (Dataset dataset : datasets) dataset.allocateSegments();
  ...
  private Optional<Org> getDsorg(int fileNbr) {
    ...
    if (opt.isEmpty()) opt = getInmr02(fileNbr, "INMCOPY");
    if (opt.isPresent()) {
      ControlRecord controlRecord = opt.get();
      TextUnit textUnit = controlRecord.getTextUnit(TextUnit.INMDSORG);
      if (textUnit != null) return Optional.of(((Dsorg) textUnit).type);
    }
  ...
  private Optional<ControlRecord> getInmr02(int fileNbr) {
    ...
    for (ControlRecord controlRecord : controlRecords)
      if (controlRecord.fileNbrMatches(fileNbr)) return Optional.of(controlRecord);
  ...
  private Optional<ControlRecord> getInmr02(int fileNbr, String utilityName) {
    ...
    for (ControlRecord controlRecord : controlRecords)
      if (controlRecord.fileNbrMatches(fileNbr)) {
        ...
        if (textUnit != null && ((TextUnitString) textUnit).getString().equals(utilityName))
