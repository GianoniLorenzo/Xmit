...
public abstract class Segment {
  ...
  public abstract byte[] getRawBuffer(); // contains no headers
  ...
  public void addBlockPointer(BlockPointer blockPointer) {
    ...
    if (blockPointer.offset + blockPointer.length > blockPointer.buffer.length) {
      ...
      System.out.printf(
          "%06X  %02X  %06X%n",
          blockPointer.offset, blockPointer.length, blockPointer.buffer.length);
      return;
    }
  ...
  public String toString() {
    ...
    return String.format(
        "%06X:   %06X  %<,7d  %,5d", blockPointer.offset, rawBufferLength, rawBlockPointers.size());
  }
}
