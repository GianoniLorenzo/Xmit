...
public class CodePage {
  ...
  public static void main(String[] args) {
    ...
    if (true) {
      ...
      for (int i = 0; i < 256; i++) {
        if (allSame(codePages, i)) continue;
        System.out.printf("%02X:   ", i);
        for (CodePage codePage : codePages) System.out.printf("%02X     ", codePage.ebc2asc[i]);
        if (codePages[5].ebc2asc[i] != codePages[6].ebc2asc[i]) System.out.print(" **");
        System.out.println();
      }
    }
  }
  ...
  private static boolean allSame(CodePage[] codePages, int i) {
    ...
    for (int j = 1; j < codePages.length; j++) if (codePages[j].ebc2asc[i] != base) return false;
    return true;
  }

  private static boolean noChange(CodePage[] codePages, int i) {

    for (CodePage codePage : codePages) if (codePage.ebc2asc[i] != i) return false;
    return true;
  }
  ...
  private static void list() {
    ...
    for (Charset charset : Charset.availableCharsets().values()) {
      System.out.printf(
          "%-15s %-15s %-6s", charset.name(), charset.displayName(), charset.canEncode());
      for (String aliasName : charset.aliases()) System.out.printf(" %-15s", aliasName);
      System.out.println();
    }
  }
  ...
  public String toString() {
    ...
    for (int i = 0; i < 256; i += 32) {
      for (int j = 0; j < 32; j++) text.append(String.format("%02X ", ebc2asc[i + j]));
      text.append("\n");
    }
