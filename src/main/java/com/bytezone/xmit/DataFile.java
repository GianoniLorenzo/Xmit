...
public abstract class DataFile implements Comparable<DataFile> {
  ...
  public List<String> getLines() {
    ...
    if (lines.size() == 0 || codePage != Utility.getCodePage()) createDataLines();
  ...
  private void createLines() {
    ...
    if (Utility.isBinary(buffer, 0, 128))
      Collections.addAll(lines, Utility.getHexDump(buffer).split("\n"));
    else createTextLines(buffer);
  }
  ...
  private void xmitLines() {
    ...
    try {
      ...
      if (dataset.isPartitionedDataset()) {
        ...
        lines.add("--------  --------  ------  -----------  --------  --------");
        for (CatalogEntry catalogEntry : (PdsDataset) dataset) lines.add(catalogEntry.toString());
      }
    } catch (Exception e) {
